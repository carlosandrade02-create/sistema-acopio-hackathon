/* ============================================================
   CircularGallery — port a JavaScript vanilla del componente de React Bits
   (https://reactbits.dev). Usa `ogl` cargado como módulo ES desde un CDN.
   Se monta en la landing, sección "Quiénes participan" (`.cg[data-cg]`).
   Si no hay internet y `ogl` no carga, el contenedor conserva su
   contenido de respaldo (una cuadrícula de fotos) y no pasa nada.
   ============================================================ */
  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      vertex: `
        ...
        void main() {
          vUv = uv;
          // Planos totalmente rígidos: sin ondulación / temblor al desplazar.
...
class App {
  constructor(container, { items, bend = 3, textColor = '#ffffff', borderRadius = 0, font = 'bold 30px sans-serif', scrollSpeed = 2, scrollEase = 0.05 } = {}) {
  ...
  addEventListeners() {
    ...
    // Los eventos de rueda/arrastre se limitan al contenedor para no
...
function mount(el) {
  let items;
  try { items = JSON.parse(el.dataset.cg); } catch (e) { return; }
  if (!items || !items.length) return;
  el.innerHTML = '';
  const app = new App(el, {
    items, bend: 2.4, textColor: '#ffffff', borderRadius: 0,
    font: '800 22px Montserrat, sans-serif', scrollSpeed: 1.5, scrollEase: 0.05
  });
  app._el = el;
  live.add(app);
}
...
function mountAll() {
  document.querySelectorAll('.cg[data-cg]').forEach(el => {
    if (el.dataset.cgDone) return;
    el.dataset.cgDone = '1';
    try { mount(el); } catch (e) { console.error('CircularGallery:', e); delete el.dataset.cgDone; }
  });
}

function sweep() {
  for (const app of [...live]) {
    if (!document.body.contains(app.container)) { app.destroy(); live.delete(app); }
  }
}
...
function remount() {
  for (const app of [...live]) {
    ...
    app.destroy();
    live.delete(app);
    if (el) { delete el.dataset.cgDone; }
  }
  mountAll();
}

window.CircularGallery = { mountAll, remount };

const root = document.getElementById('root');
if (root) {
  new MutationObserver(() => { sweep(); mountAll(); }).observe(root, { childList: true, subtree: true });
}
// Recolorear las etiquetas al cambiar de tema claro/oscuro
let lastTheme = document.documentElement.dataset.theme;
new MutationObserver(() => {
  if (document.documentElement.dataset.theme !== lastTheme) {
